  ...
  def _get_params(ret, candidate, fingerprint, car_fw, disable_openpilot_long, experimental_long, docs):
    ...
    if candidate in (CAMERA_ACC_CAR | SDGM_CAR):
      ...
      if cslcEnabled:
        ...
        ret.openpilotLongitudinalControl = True
        ret.stoppingDecelRate = 3.25  # 5.2Kmh == 8.33 mph/s (OFF + ON = 12 frames)
        if candidate in SDGM_CAR:
          ret.stoppingDecelRate = 7.45  #12Kmh  == 16.67 mph/s (OFF + ON = 30 frames)
    ...
    if candidate in (CAR.CHEVROLET_VOLT, CAR.CHEVROLET_VOLT_CC):
      ...
      ret.longitudinalTuning.kf = 1.  # get_steer_feedforward_volt()
